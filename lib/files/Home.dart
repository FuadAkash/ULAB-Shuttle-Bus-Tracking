...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text('Bus Tracking - $displayName')),
      body: GoogleMap(
        initialCameraPosition: CameraPosition(target: LatLng(23.8041, 90.4152), zoom: 14),
        markers: Set<Marker>.of(_driverMarkers),
        /*onTap: (LatLng latLng) {
          // Find the tapped marker using the latLng
          Marker? tappedMarker = _driverMarkers.firstWhere(
                (marker) => marker.position == latLng,
            orElse: () => null,
          );

          if (tappedMarker != null) {
            String busDriverId = tappedMarker.markerId.value; // Use marker ID as busDriverId
            sendPickupRequest(busDriverId); // Call the function to send the pick-up request
            print(busDriverId);
          }
        },*/
